...
import { ChevronDown, FileAudio, Folder, FolderOpen, SlidersHorizontal } from "lucide-react";
...
export function MainForm({ formId, settings, onSubmit, onSettingsChange, onChooseInput, onChooseOutput, submitDisabled = false }: Props) {
  ...
  return <form id={formId} className="space-y-4" onSubmit={(event) => { event.preventDefault(); submit(); }}>
    <fieldset><legend className="mb-2 text-sm font-medium text-slate-700">{t("main.input.type")}</legend><div className="grid grid-cols-2 rounded-lg bg-slate-100 p-1">{(["file", "folder"] as const).map((mode) => <label key={mode} className={cn("flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-md text-sm font-medium focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-focus", inputMode === mode ? "bg-selected text-slate-900 shadow-sm ring-1 ring-inset ring-primary" : "text-slate-600 hover:bg-white/70 hover:text-slate-900 active:bg-white")}><input className="sr-only" type="radio" name="input-mode" value={mode} checked={inputMode === mode} onChange={() => setInputMode(mode)} /><span className={inputMode === mode ? "text-primary" : undefined}>{mode === "file" ? <FileAudio className="size-4" /> : <Folder className="size-4" />}</span>{t(`main.input.${mode}`)}</label>)}</div></fieldset>
...
function CheckOption({ checked, onChange, label }: { checked: boolean; onChange: (value: boolean) => void; label: string }) { return <label className="flex cursor-pointer items-center gap-3 text-sm"><input className="size-4 accent-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus" type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} />{label}</label>; }
function SelectOption({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: [string, string][] }) { return <label className="block text-sm font-medium text-slate-700">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm focus-visible:border-focus focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus">{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></label>; }
